        ...
        //监听当键将要退出时
         [[NSNotificationCenter defaultCenter] addObserver:self
                                                  selector:@selector(keyboardWillHide:)
                                                    ...
                                                    object:nil];
        [self layoutUI];
        NSLog(@"videodada=%@",self.videoId);
//        [self.commentListView loadDataWithVideoId:[self.videoId intValue]];
//        [self showView];
    }
    return self;
}
- (void)layoutUI{
    ...
    _commentListView.videoId=self.videoId;
     [self.commentListView loadDataWithVideoId:[self.videoId intValue]];
    NSLog(@"%@",self.videoId);
...
- (void)endEdit{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        self.commentListView.toolView.frameY = SCREEN_HEIGHT-ZMCusComentBottomViewHeight-SAFE_AREA_BOTTOM-STATUS_AND_NAV_BAR_HEIGHT-40*KScaleH;
...
- (void)showViewWithVideoId:(int)vidoId{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        self.commentListView.videoId=[NSString stringWithFormat:@"%d",vidoId];
        [self.commentListView loadDataWithVideoId:vidoId];
        NSLog(@"aaaaaaaq====%d",vidoId);
    } completion:^(BOOL finished) {
