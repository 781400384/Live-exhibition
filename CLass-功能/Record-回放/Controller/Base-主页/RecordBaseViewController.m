...
@interface RecordBaseViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
-(void)loadData{
    [RecordHandle getRecordListWiithPage:self.page success:^(id  _Nonnull obj) {
        NSDictionary * dic=(NSDictionary *)obj;
               if ([dic[@"code"] intValue]==200) {
                   if (self.page==1) {
                   self.dataList=[RecordMainModel mj_objectArrayWithKeyValuesArray:dic[@"data"]];
                   }else{
               [self.dataList addObjectsFromArray:[RecordMainModel mj_objectArrayWithKeyValuesArray:dic[@"data"]]];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([UserInfoDefaults isLogin]) {
        ...
        RecordMainModel * model=self.dataList[indexPath.item];
        vc.typeId=model.exhibition_id;
        vc.naviView.naviTitleLabel.text=@"回放";
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.recordModel=self.dataList[indexPath.row];
    return  cell;
}
